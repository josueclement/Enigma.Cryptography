...
public class TripleDesCbcTests
{
    ...
    public async Task CsvEncryptTest(byte[] key, byte[] iv, byte[] data, byte[] encrypted)
    {
        ...
        var service = new BlockCipherServiceFactory().CreateCbcService(engineFactory.CreateTripleDesEngine);
    ...
    public static IEnumerable<object[]> GetCsvValues()
    {
        ...
        return File.ReadAllLines(Path.Combine("BlockCiphers", "tripledes-cbc.csv"))
            .Skip(1)
            .Select(line =>
            {
