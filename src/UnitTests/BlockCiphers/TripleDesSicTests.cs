...
public class TripleDesSicTests
{
    [Theory]
    ...
    public async Task CsvEncryptTest(byte[] key, byte[] iv, byte[] data, byte[] encrypted)
    {
        ...
        var service = new BlockCipherServiceFactory().CreateSicService(engineFactory.CreateTripleDesEngine);
    ...
    public static IEnumerable<object[]> GetCsvValues()
    {
        ...
        return File.ReadAllLines(Path.Combine("BlockCiphers", "tripledes-ctr.csv"))
            .Skip(1)
            .Select(line =>
            {
