...
public class DesSicTests
{
    ...
    public async Task CsvEncryptTest(byte[] key, byte[] iv, byte[] data, byte[] encrypted)
    {
        ...
        var service = new BlockCipherServiceFactory().CreateSicService(engineFactory.CreateDesEngine);
    ...
    public static IEnumerable<object[]> GetCsvValues()
    {
        ...
        return File.ReadAllLines(Path.Combine("BlockCiphers", "des-ctr.csv"))
            .Skip(1)
            .Select(line =>
            {
