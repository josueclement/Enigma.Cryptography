...
public class ChaCha20Tests
{
    ...
    {
        var service = new StreamCipherServiceFactory().CreateChaCha20Service();
        
        using var msInput = new MemoryStream(data);
        using var msOutput = new MemoryStream();

        await service.EncryptAsync(msInput, msOutput, key, nonce, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CsvDecryptTest(byte[] key, byte[] nonce, byte[] data, byte[] encrypted)
    {
        ...
        await service.DecryptAsync(msInput, msOutput, key, nonce, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public static IEnumerable<object[]> GetCsvValues()
    {
        ...
        return File.ReadAllLines(Path.Combine("StreamCiphers", "chacha20.csv"))
            .Skip(1)
            .Select(line =>
            {
